...
static void prvSetupTimerInterrupt( void )
{
	/* timer is selected in counter mode with a period set to
	   match the desired tick as closely as possible */
	uint32_t ulPeriod = configCPU_CLOCK_HZ / configTICK_RATE_HZ;;
	uint8_t  ucPrescaler = 1;
	//1,2,3,4,5, 6,  7
	//1,2,4,8,64,256,1024
	//1 1 1 1 3  2   2

	/* minimum prescaler is given by the high word */
	while (ulPeriod & 0xFFFF0000)
	{
		switch (ucPrescaler)
		{
			case 4: ulPeriod >>=1;
	...
	/* maximum prescaler given by remaining cleared LSBs */
	/* TODO: calculate for better prescaling */

	/* setup the registers */
	TCC1.CTRLA = ucPrescaler;
	TCC1.PER = (uint16_t)ulPeriod;
